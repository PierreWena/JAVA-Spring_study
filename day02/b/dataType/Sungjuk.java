...
public class Sungjuk {
	...
	*/	
	...
	String hakbun; // 학번  null  "12345"  "20191234"  "091234"  "013456"
	               // 데이터 값이 숫자로만 이루어지더라도 맨 앞에 0 이 올수 있는 경우라면
	               // String 타입으로 해야 한다.
	
	String name;   // 학생명 null
	
	byte korPoint;  // 국어점수 0 카멜표기법 : 단어와 단어가 결합된 경우이라면 두번째 단어는 대문자로 시작하도록 하는 것. 
	byte eng_point; // 영어점수 0 스네이크표기법 : 단어와 단어가 결합된 경우이라면 단어 사이에 _ 를 붙여주는 것. 
    ...
    void showSungjuk() {
    	...
    	short total = (short) (korPoint + eng_point + mathpoint);
    	...
    	// (short) (korPoint + eng_point + mathpoint) ==> int 타입을 강제로 short 타입으로 형변환(casting) 시킨것. 강제형변환(casting)이라고 부른다.  
    ...
    //	double avg = total/3.0;  // 정수/실수 ==> 실수 
    	...
    	switch ( (total/3)/10 ) {  //   300/3    299/3    288/3    270/3     268/3    260/3   240/3 
			...
			// 주의할 점은 switch(비교대상) 에서 비교대상의 데이터타입은 byte, short, int, char, String 타입만 들어올 수 있다.!!!!
            // 비교대상의 데이터타입에는 long, float, double 은 들어올 수 없다.!!!!    	
    	...
    	char ch1 = 'k';
    	char ch2 = 107;
    	char ch3 = 'k'+2; // 107+2 == 109
    	                  // char 타입에 사칙연산(+ - * /)이 붙으면 자동적으로 char 타입은 int 타입으로 형변환 된다.
    	
    	char ch4 = '\u231B'; //  \\u 은 유니코드를 뜻한다.
